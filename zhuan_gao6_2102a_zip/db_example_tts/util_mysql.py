...
if '__main__' == __name__:
    ...
    def _main():
        ...
        ts = time.time_ns()
        enqueue(db, 'dialog_out', 'uyyyy', str(ts), {
            VALUE: '聊天内容yyyy的内容。',
            'io': 'yy',
        })

        ts = time.time_ns()
        cnt = mysql_insert_one(db, 'dialog_out', 'uxxxx', str(ts), {
            VALUE: '聊天内容001的内容。',
            'io': 'io',
        })
        print('cnt:', cnt)

        cnt = mysql_update_one(db, 'dialog_out', 'uxxxx', '1694356029900504400', {
            VALUE: '更新的内容！！！',
            'io': 'xx',
        })
        print('upd cnt:', cnt)

        xrow = mysql_get_one(db, 'dialog_out', 'uxxxx', '1694356029900504400')
        print('row:', xrow)

        xrows = get_sorted_by_key(db, 'dialog_out', 'uxxxx', limit=5, is_keep_others=False)
        print(xrows)
